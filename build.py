...
class Model(RADDCore):
    ...
    def __init__(self, data=pd.DataFrame, kind='dpm', inits=None, fit_on='average', depends_on=None, niter=50, fit_noise=False, fit_whole_model=True, tb=None, weighted=True, pro_ss=False, dynamic='hyp', tol=1.e-10, split=50, verbose=False, include_zero_rts=False, *args, **kws):
        ...
        super(Model, self).__init__(data=self.data, inits=inits, fit_on=fit_on, depends_on=depends_on, niter=niter,
                                    fit_whole_model=fit_whole_model, kind=kind, tb=tb, fit_noise=fit_noise, pro_ss=pro_ss, split=split, dynamic=dynamic)
    ...
    def make_optimizer(self, inits=None, ntrials=10000, tol=1.e-5, maxfev=5000, disp=True, bdisp=False, multiopt=True, nrand_inits=2, niter=40, interval=10, stepsize=.05, nsuccess=20, method='TNC', btol=1.e-3, maxiter=20):
        ...
        """
        fp = self.set_fitparams(
            tol=tol, maxfev=maxfev, ntrials=ntrials, niter=niter, disp=disp, get_params=True)
        bp = self.set_basinparams(btol=btol, interval=interval, niter=niter, maxiter=maxiter, method=method,
                                  nsuccess=nsuccess, stepsize=stepsize, nrand_inits=nrand_inits, bdisp=bdisp, get_params=True)
        ...
        self.opt = fit.Optimizer(dframes=self.dframes, fitparams=fp, basinparams=bp, kind=self.kind, inits=inits,
                                 depends_on=self.depends_on, fit_on=self.fit_on, wts=self.avg_wts, pc_map=self.pc_map,  multiopt=multiopt)
    ...
    def optimize(self, inits=None, y=None, ntrials=10000, tol=1.e-5, maxfev=5000, disp=True, bdisp=False, nrand_inits=2, niter=40, interval=10, stepsize=.05, nsuccess=20, method='TNC', btol=1.e-3, maxiter=20, multiopt=True, stage='full', save=True, savepth='./'):
        ...
        self.make_optimizer(inits=inits, ntrials=ntrials, tol=tol, maxfev=maxfev, disp=disp, bdisp=bdisp, nrand_inits=nrand_inits, niter=niter,
                            method=method, multiopt=multiopt, btol=btol, interval=interval, stepsize=stepsize, nsuccess=nsuccess, maxiter=maxiter)

        if stage == 'flat':
            y, fi, p = self.opt.optimize_flat(
                p0=inits, y=y, random_init=multiopt)
            self.flat_fits, self.flat_fitinfo, self.flat_popt = y, fi, p
        elif stage == 'cond':
            if inits is None and hasattr(self, 'flat_popt'):
                inits = self.flat_popt
            y, fi, p = self.opt.optimize_conditional(
                p=inits, y=y, precond=multiopt)
    ...
    def make_simulator(self, p=None):
        ...
        self.simulator = models.Simulator(
            fitparams=self.fitparams, kind=self.kind, inits=p, pc_map=self.pc_map)
    ...
    def prepare_fit(self):
        ...
        * Parameters (p[pkey]=pval) that are constant across conditions
        ...
        if self.verbose:
            self.is_prepared = saygo(depends_on=self.depends_on, labels=self.labels,
